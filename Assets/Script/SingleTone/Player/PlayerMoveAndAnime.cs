...
public class PlayerMoveAndAnime : Singleton<PlayerMoveAndAnime>
{
    ...
    public InterfaceView __interfaceView { get { return interfaceView; } set { interfaceView = value; } }
    public Joystick __joystick { get { return joystick; } set { joystick = value; } }
    ...
    private void Interact_With_Object()
    {
        ...
        {
            interfaceView.InteractButton_Off();
        }
    }
    ...
    {
        // 조이스틱 벡터 입력
        joystickVec = __joystick.Direction;
    ...
    /* 키보드 조작관련
    private void Move_And_Animation_KeyBoard()
    {
        int keyCodeInt = 0;
        ...
        if (Input.GetKeyDown(KeyCode.W))
        {
            ...
            interfaceView_script.ChangeColer(keyCodeInt);
            animator.SetBool(animation_MoveKey, true);

            curMoveVec = Vector2.up;
        }
        else if (Input.GetKeyUp(KeyCode.W))
        {
            keyCodeInt = (int)KeyCode.W;
            interfaceView_script.revertColor(keyCodeInt);
            animator.SetBool(animation_MoveKey, false);

            curMoveVec.y -= curMoveVec.y;
        }
        ...
        // 애니메이션 코드
        animator.SetInteger("curState", keyCodeInt);

        // 캐릭터 움직임
        rigid.AddForce(curMoveVec * moveSpeed / 10, ForceMode2D.Impulse);
        if (curMoveVec == Vector2.zero)
        {
            rigid.linearVelocity = Vector2.zero;
        }
    }
    ...
    {
        if (Input.GetKeyDown(KeyCode.Space) && interfaceView_script.isInteractiveOn)
        {
            interfaceView_script.ChangeColer((int)KeyCode.Space);
        ...
        {
            interfaceView_script.revertColor((int)KeyCode.Space);
        }
    }
    */
}
